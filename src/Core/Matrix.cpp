...
///////////////////////////////////////////////////////////////////////////////
Matrix4& Matrix4::LookAt(const Vec3f& target)
{
	// compute forward vector and normalize
	Vec3f position = Vec3f(m[12], m[13], m[14]);
	Vec3f forward = target - position;
	forward.Normalize();
	Vec3f up;             // up vector of object
	Vec3f left;           // left vector of object
	...
	// compute left vector
	left = Vec3f::CrossProduct(up, forward);
	left.Normalize();

	// re-compute up vector
	up = Vec3f::CrossProduct(forward, left);
	//up.normalize();
...
Matrix4& Matrix4::LookAt(const Vec3f& target, const Vec3f& upVec)
{
	// compute forward vector and normalize
	Vec3f position = Vec3f(m[12], m[13], m[14]);
	Vec3f forward = target - position;
	...
	// compute left vector
	Vec3f left = Vec3f::CrossProduct(upVec, forward);
	left.Normalize();

	// compute orthonormal up vector
	Vec3f up = Vec3f::CrossProduct(forward, left);
	up.Normalize();
